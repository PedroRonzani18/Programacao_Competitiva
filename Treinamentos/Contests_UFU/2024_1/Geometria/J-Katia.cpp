...
void solve(int a, int b) {
	...
	double azinho1 = (a + b + sqrt(a*a-a*b+b*b)) / 6.;
	double azinho2 = (a + b - sqrt(a*a-a*b+b*b)) / 6.;

	double v1 = (a - 2*azinho1) * (b - 2*azinho1) * azinho1;
	double v2 = (a - 2*azinho2) * (b - 2*azinho2) * azinho2;

	cout << fixed << setprecision(4);

	if(v1 > v2) {
		cout << azinho1 << " " << v1 << endl;
	} else {
		cout << azinho2 << " " << v2 << endl;
	}
