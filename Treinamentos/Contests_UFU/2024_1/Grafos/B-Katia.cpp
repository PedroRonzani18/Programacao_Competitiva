...
void solve() {
	...
	vi infectantes;
	vi infectadoPorQuem(n+1, -1);	

	while(c--) {
		int infectante, nums; cin >> infectante >> nums;
		infectantes.push_back(infectante);
		while(nums--) {
			int x; cin >> x;
			if(infectadoPorQuem[x] == -1)
				infectadoPorQuem[x] = infectante;
		}
	}

	for(int inf : infectantes) {
		if(infectadoPorQuem[inf] == -1) {
			cout << inf << endl;
			break;
		}
	}
