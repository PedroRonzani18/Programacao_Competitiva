...
vector<int> dijkstra(Graph &g, vector<int> ms) // retorna todas as distancias entre todos os nodes e os multisources
{
...
void solve()
{
   ...
   vector<int> ms(q); // conjunto de vertices que começa dijksrta

   for(int& i : ms)
      cin >> i;

   vector<int> dist = dijkstra(grp, ms);
