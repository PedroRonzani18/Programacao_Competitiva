...
2a7 const int MAXN = 1e4+10;
d95 const int MAXEDG = 1e4+10;

9d3 int dist[MAXN];
8aa vector<int> niveisDoNode[MAXN], itensDoNivel[MAXEDG];
...
fce             for(auto u : itensDoNivel[nivel]) {
619                 if (dist[u] == 0) {
324                     q.push({u, dis+1});
554                     dist[u] = dis + 1;
12f                 }
...
98f 	int n, ed;
31c 	f(i,0,ed) {
509 		int q; cin >> q; // quantidade de elementos nesse nivel
    
1f4 		while(q--) {
1b3             int v; cin >> v;
e19 			niveisDoNode[v].push_back(i);
59a 			itensDoNivel[i].push_back(v);
e5f         }
e3b 	}
