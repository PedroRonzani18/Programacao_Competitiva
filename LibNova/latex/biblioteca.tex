...
7c9         int ret = 0;
be8         for (i++; i; i -= i&-i)
1bd             ret += lower_bound(bit[i].begin(), bit[i].end(), k) - bit[i].begin();
edf         return ret;
6f9     }
690     int query(int l, int r, T k) {
83d         return p_query(r, k) - p_query(l-1, k);
bcc     }
8d0 };
\end{lstlisting}
...
\subsection{RMQ <O(n), O(1)> - min queue}
\begin{lstlisting}
...
c92 	int small(int r, int sz = b) { return r-msb(mask[r]&((1<<sz)-1)); }
6ad 	rmq() {}
...
39d 		for (int j = 1; (1<<j) <= n/b; j++) for (int i = 0; i+(1<<j) <= n/b; i++)
...
a4e 		int j = msb(y-x+1);
ea3 		int ans = op(small(l+b-1), op(t[n/b*j+x], t[n/b*j+y-(1<<j)+1]));
...
16d 		if (cs.size() == 1) return fhash(cs[0], -1); // se forem enraizadas, chamar esse direto com o vertice raiz
...
367 		else idx = x = find(d.begin()+idx+1, d.end(), 1) - d.begin();
5f9 	}
edf 	return ret;
d3b }
...
8d6 int sub(int a, int b, int mod) { return ((a - b) % mod + mod) % mod; }
...
\subsection{Integracao Numerica}
\begin{lstlisting}
...
dd1 	for (int a : {2, 325, 9375, 28178, 450775, 9780504, 1795265022}) {
da0 		ll x = pow(a, d, n);
...
\subsection{Primos - IsPrime - Miinter-Rabin}
\begin{lstlisting}
...
\subsection{Primos - IsPrime - Tradicional}
\begin{lstlisting}
...
\subsection{Primos - Soma Divisores}
\begin{lstlisting}
